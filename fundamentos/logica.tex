...
\begin{itemize}[noitemsep]
    \item \textbf{Cuantificador Universal ($\forall$):} La proposición $\forall x \in T, P(x)$ significa que $P(x)$ es verdadero para todo elemento de tipo $T$. Se demuestra asumiendo un $x$ arbitrario y se refuta con un solo \textbf{contraejemplo}. Si no existen elementos en $T$, la proposición es \textbf{vacuamente verdadera}.
    \item \textbf{Cuantificador Existencial ($\exists$):} La proposición $\exists x \in T : P(x)$ significa que existe al menos un $x$ para el cual $P(x)$ es verdadero. Basta con exhibir un ejemplo específico para probarla.
\end{itemize}

Decir que algo es cierto para todo $x$ es una afirmación mucho más fuerte que decir que es cierto para algún $x$, excepto cuando el dominio es vacío (donde el $\forall$ es vacuo y el $\exists$ es falso). Ya habíamos hablado de negación de ciertos tipos de conectores lógicos, ahora tenemos que la negación de una proposición universal produce una existencial y viceversa:
\begin{itemize}[noitemsep]
