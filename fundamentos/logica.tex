...
\begin{nota}
    Aunque en esencia un enunciado y una proposición son lo mismo, en estas notas las \textit{proposiciones} son enunciados más complejos, enunciados primitivos conectados lógicamente.
\end{nota}
...
\begin{nota}
    Este método también se conoce como demostración por contradicción y lo veremos a detalle más adelante. Es importante añadir que, aunque usar los conectores lógicos parezca sencillo, muchas veces los aplicamos de forma incorrecta sin darnos cuenta. Por eso, es fundamental practicar con ejercicios de lógica y tablas de verdad; cualquier libro de matemáticas elementales incluye estas herramientas, que son ideales para evitar errores de razonamiento y entender bien cómo funcionan estas estructuras.
\end{nota}
...
\begin{proof}
    Asumimos $A$ es verdadero. Como $A$ es verdadero, $C$ es verdadero. Como $C$ es verdadero, $D$ es verdadero. Como $D$ es verdadero, $B$ es verdadero. 
\end{proof}
...
El orden de los cuantificadores es irrelevante cuando son del mismo tipo, de modo que $\forall x \forall y P(x, y) \equiv \forall y \forall x P(x, y)$ y $\exists x \exists y P(x, y) \equiv \exists y \exists x P(x, y)$. No obstante, en cuantificadores mixtos el orden es fundamental porque define la dependencia de las variables: en la secuencia $\forall n \exists m (m > n)$ sobre el dominio $\mathbb{Z}$, la afirmación es verdadera porque para cada $n$ se puede elegir un $m$ distinto (como $m = n + 1$), mientras que $\exists m \forall n (m > n)$ es falsa al requerir un único valor de $m$ que sea superior a todo el conjunto. En términos generales, en la secuencia $\forall x \exists y$ el valor de $y$ se elige después de conocer $x$, mientras que en $\exists y \forall x$, la variable $y$ debe ser una constante universal válida para todos los valores de $x$ simultáneamente.
